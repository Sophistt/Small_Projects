...
class PurePursuit(object):
    ...
    def trajectoryTracking(self, pathPoints, currentPoint, direction):
        ...
        for i in range(200):
            calDistance = abs(
                pathPoints[i].x - currentPoint.x - self.lookaheadDistance)
            if calDistance < tmpDistance:
                tmpDistance = calDistance
                trackPoint = deepcopy(pathPoints[i])

        goalPoint = Point(self.pixelCoordsToVehicleCoords(
            currentPoint, trackPoint, direction))
    ...
    def getAngle(self, currentPoint, trackPoint):
        ...
        angle = math.atan((currentPoint.y - trackPoint.y) /
                          (trackPoint.x - currentPoint.x))
